...
<%@ page import="model.Cause,dao.CauseDAO,java.util.List" %>
<%
if (request.getAttribute("causes") == null) {
        List<model.Cause> causes = dao.CauseDAO.getAllCauses();
        request.setAttribute("causes", causes);
        int totalCauses = dao.CauseDAO.getTotalCauses();
        int activeCauses = dao.CauseDAO.getCountActiveCauses();
        int pastCauses = dao.CauseDAO.getCountPastCauses();
        double totalDonations = 0;
        for (model.Cause c : causes) { totalDonations += c.getTotalCollected(); }
...
<body>
  ...
  <div class="alert alert-danger">${errorMessage}</div>
      ...
      <div style="display: flex; gap: 32px; flex-wrap: wrap; margin-bottom: 32px;">
        <div style="flex: 1; min-width: 320px; background: #fff; border-radius: 18px; box-shadow: 0 2px 12px rgba(0,0,0,0.07); padding: 24px;">
          <h5 style="text-align:center; font-weight:700;">Total Collected Per Cause</h5>
          <canvas id="causePieChart" height="220"></canvas>
        </div>
        <div style="flex: 2; min-width: 420px; background: #fff; border-radius: 18px; box-shadow: 0 2px 12px rgba(0,0,0,0.07); padding: 24px;">
          <h5 style="text-align:center; font-weight:700;">Target vs Collected Amounts</h5>
          <canvas id="causeBarChart" height="220"></canvas>
        </div>
      </div>
    ...
    <script>
      // Prepare data from JSP EL
      var causes = [
        <c:forEach var="cause" items="${causes}" varStatus="loop">
          {
            title: "${cause.title}",
            collected: ${cause.totalCollected},
            target: ${cause.targetAmount}
          }<c:if test="${!loop.last}">,</c:if>
        </c:forEach>
      ];
      // Pie chart: Total Collected Per Cause
      window.addEventListener('DOMContentLoaded', function() {
        var pieCtx = document.getElementById('causePieChart').getContext('2d');
        var pieChart = new Chart(pieCtx, {
          type: 'pie',
          data: {
            labels: causes.map(c => c.title),
            datasets: [{
              data: causes.map(c => c.collected),
              backgroundColor: [
                '#4b49ac', '#f6c23e', '#e74a3b', '#36b9cc', '#1cc88a', '#858796', '#fd7e14', '#20c997', '#6f42c1', '#e83e8c'
              ]
            }]
          },
          options: {
        ...
        var barCtx = document.getElementById('causeBarChart').getContext('2d');
        var barChart = new Chart(barCtx, {
          ...
          data: {
            ...
            datasets: [
              {
                label: 'Target Amount',
                data: causes.map(c => c.target),
                backgroundColor: 'rgba(75, 73, 172, 0.5)'
              },
              {
